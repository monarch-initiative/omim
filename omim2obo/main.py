...
def run(use_cache: bool = False):
    ...
    for mim_number, mim_data in morbid_map.items():
        ...
        # TODO: RO:0003302 for multiple mapping evidence https://github.com/monarch-initiative/omim/issues/75
        #  - rather than for loop, do if/else on len(assocs)
        # len(set([x['phenotype_label'] for x in assocs])) > 1

        # TODO: analysis: save this file in /dev
        # https://github.com/monarch-initiative/omim/issues/86
        problematic = {}
        for mim, data in morbid_map.items():
            assocs: List[Dict] = data['phenotype_associations']
            unique_assocs = set([x['phenotype_mim_number'] for x in assocs])
            if '' in unique_assocs and len(unique_assocs) > 1:
                problematic[mim] = data
        problematic_mims = [int(x) for x in problematic.keys()]

        import pandas as pd
        df = pd.read_csv(os.path.join(ROOT_DIR, 'data', 'morbidmap.tsv'), sep='\t', comment='#')
        df = df[df['MIM Number'].isin(problematic_mims)]
        df = df.sort_values(['MIM Number'])
        df.to_csv('~/Desktop/morbidmap cases of mim mapped to both another mim and a plain label.tsv', index=False, sep='\t')


        print()

        # TODO: check: is it ever the case that there is a common MIM in the "MIM Number" column of multiple rows,
        #  ...and some of the rows map to a "Phenotype" field with a MIM number, while others have no MIM number.
        for
        ...
        # TODO: Use different predicates based on mapping evidence

        graph.add((b, OWL['onProperty'], RO['0003303']))
        if p_mim:
        ...
        # ii. add axiom
        b2 = BNode()
        # TODO: add biolink:evidence back https://github.com/monarch-initiative/omim/pull/72
        ...
        # todo: add biolink:GeneDiseaseAssociation structure?
        # todo: add mapping key and related info in a different way?
        # TODO: convert evidence = list(set([x['p_map_key'] for x in assocs])) (and same for p_map_lab) into a
        #  single string and add that into Literal(). Almost 100% of these evidence will be the same key/lab.
        #  However, make it Literal('; '.join(evidence)) because there is 1 case in which multiple evidences
        ...
        graph.add((b3, OWL['onProperty'], RO['0003303']))
        if p_mim:
